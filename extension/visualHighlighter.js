...
export function highlightSegment(segmentScore, options = {}) {
  ...
  if (showTooltipOnClick) {
    // Attach handlers and keep references to remove later (prevent duplicates)
    const onClick = (e) => { e.stopPropagation(); showExplanationTooltip(element, segmentScore); };
    element.addEventListener('click', onClick);

    // Store handlers for later removal
    element.__sharesafeHandlers = element.__sharesafeHandlers || {};
    element.__sharesafeHandlers.click = onClick;

    // Also show on hover for high-risk segments
    if (segmentScore.riskLevel === 'high') {
      let hoverTimeout = null;
      const onMouseEnter = () => {
        hoverTimeout = setTimeout(() => { showExplanationTooltip(element, segmentScore); }, 500);
        element.__sharesafeHoverTimeout = hoverTimeout;
      };
      const onMouseLeave = () => {
        if (element.__sharesafeHoverTimeout) {
          clearTimeout(element.__sharesafeHoverTimeout);
          element.__sharesafeHoverTimeout = null;
        }
      ...
      element.__sharesafeHandlers.mouseenter = onMouseEnter;
      element.__sharesafeHandlers.mouseleave = onMouseLeave;
    }
  }
}
...
export function showExplanationTooltip(anchorElement, segmentScore) {
  ...
  tooltip.innerHTML = `
    ...
    <div class="sharesafe-explanation-reasons">
      ...
      <div class="sharesafe-explanation-reasons-list"></div>
    </div>
  ...
  // Safely populate reasons using DOM methods to avoid XSS
  try {
    const reasonsList = tooltip.querySelector('.sharesafe-explanation-reasons-list');
    const reasons = Array.isArray(segmentScore.reasons) ? segmentScore.reasons : [];
    reasons.forEach(reason => {
      const reasonWrap = document.createElement('div');
      reasonWrap.className = 'sharesafe-explanation-reason';

      const iconSpan = document.createElement('span');
      iconSpan.className = 'sharesafe-explanation-reason-icon';
      iconSpan.textContent = '•';

      const textSpan = document.createElement('span');
      textSpan.textContent = String(reason);

      reasonWrap.appendChild(iconSpan);
      reasonWrap.appendChild(textSpan);
      reasonsList.appendChild(reasonWrap);
    });
  } catch (e) {
    // If safe insertion fails, continue without reasons
  }
...
export function clearAllHighlights() {
  ...
  document.querySelectorAll('.sharesafe-segment-highlight').forEach(el => {
    ...
    // Remove event listeners added by highlightSegment
    try {
      const handlers = el.__sharesafeHandlers;
      if (handlers) {
      ...
      if (el.__sharesafeHoverTimeout) {
        try { clearTimeout(el.__sharesafeHoverTimeout); } catch (e) {}
        el.__sharesafeHoverTimeout = null;
      }
      // Clean up handler references
      if (el.__sharesafeHandlers) delete el.__sharesafeHandlers;
    } catch (e) {}
