...
async function analyzeImageWithLLM(imageSrc) {
  // FIX: add a timeout and normalize responses to avoid crashes
  return new Promise((resolve) => {
    let finished = false;
    ...
    try {
      chrome.runtime.sendMessage({ type: 'ANALYZE_IMAGE', imageSrc }, (response) => {
        if (finished) return;
        finished = true;
        ...
        // Normalize the response shape to safe types
        const safe = {
          score: Number.isFinite(response.score) ? Math.max(0, Math.min(100, response.score)) : null,
          confidence: Number.isFinite(response.confidence) ? Math.max(0, Math.min(100, response.confidence)) : null,
...
function blendImageResults(metadataResult, llmResult) {
  // FIX: handle missing/nullable LLM results and normalize values
  if (!llmResult || (llmResult.score === null || llmResult.score === undefined)) {
    const safeScore = Number.isFinite(metadataResult.score) ? Math.max(0, Math.min(100, metadataResult.score)) : 0;
    const safeConfidence = Number.isFinite(metadataResult.confidence) ? Math.max(0, Math.min(100, metadataResult.confidence)) : 0;
    const reasons = Array.from(new Set((metadataResult.reasons || []).map(String))).slice(0, 6);
    return {
      score: safeScore,
      confidence: safeConfidence,
      riskLevel: safeScore >= 60 ? 'high' : safeScore >= 35 ? 'medium' : 'low',
      reasons,
      imageType: metadataResult.imageType || 'unknown',
      method: 'metadata',
      editIndicators: metadataResult.editIndicators || 0,
      syntheticIndicators: metadataResult.syntheticIndicators || 0,
      interpretation: safeScore >= 70 ? 'Likely AI-generated image' : safeScore >= 35 ? 'Uncertain - verify source' : 'No strong AI indicators'
    };
  }

  const mScore = Number.isFinite(metadataResult.score) ? metadataResult.score : 0;
  const lScore = Number.isFinite(llmResult.score) ? llmResult.score : 0;
  const blendedScore = Math.max(0, Math.min(100, (mScore * 0.6) + (lScore * 0.4)));

  const allReasons = Array.from(new Set([...(metadataResult.reasons || []).map(String), ...(llmResult.reasons || []).map(String)])).slice(0, 6);

  const confidence = Math.max(Number(metadataResult.confidence) || 0, Number(llmResult.confidence) || 0);
  const safeConfidence = Math.max(0, Math.min(100, confidence));

  const imageType = metadataResult.imageType || llmResult.imageType || 'unknown';
